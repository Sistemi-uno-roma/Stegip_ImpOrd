...
Public Class frmPrincipale_old
    ...
    Private Sub cmdEsporta_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdEsporta.Click
        Try
            ...
            If txtFileOr.Text <> "" And _
                ...
                DatiGen.DbGruppo <> "" And cboTPLav.Text <> "" Then
                'txtFileEspoAna.Text <> "" And   txtPartitario.Text <> "" 
    ...
    Private Sub AbilitaEsportazione()
        ...
        If txtFileOr.Text <> "" And _
            ...
            If TP_LAVORO = TP_LAVORO_EN.TP_ACCETTORV Then
                If (txtRifPrev.Text) <> "" Then
                    ...
                    Do While Not rsOr.EOF
                        ...
                        If Not rs.EOF Then
                            ...
                            Do While Not rs.EOF
                                ...
                                If GestRifDocDaEvadere Then
                                    If txtRifPrev.Text <> "" Then
                                        Dim iddoc As Long = Val(txtRifPrev.Tag)
                                        If iddoc <> 0 Then
                                            Dim CodArt As String = rs.Fields(5).Value
                                            Dim VarArt As String = rs.Fields(6).Value
                                            If CodArt <> "" Then
                                                If ConnettiEs() Then
                                                    Dim rsPRV As New ADODB.Recordset
                                                    rsPRV.Open("Select top 1 dbgruppo,iddocumento,idrigadoc,NumProgrRiga from docunirigheordcli " & _
                                                               "where dbgruppo='" & DatiGen.DbGruppo & "' and iddocumento=" & iddoc & _
                                                               " and codart='" & CodArt & "' and VarianteArt ='" & VarArt & "'", m_ConnEs, ADODB.CursorTypeEnum.adOpenDynamic, ADODB.LockTypeEnum.adLockOptimistic)
                                                    If Not rsPRV.EOF Then
                                                        RifPrev = txtRifPrev.Text & "." & rsPRV("NumProgrRiga").Value
                                                    End If
                                                    rsPRV.Close()
                                                    rsPRV = Nothing
                                                End If
                                        ...
                                        End If
                                    End If
                                    If Trim(RifPrev) <> "" Then
                                        RecOut = RecOut & msep & 6 ' tipo doc da evadere preventivo
                                        RecOut = RecOut & msep & RifPrev ' tipo
                                    Else
                                        RecOut = RecOut & msep & "" ' tipo doc da evadere preventivo
                                        RecOut = RecOut & msep & "" ' tipo
                                    End If
                                End If
    ...
    Private Function LeggiExcel(ByVal NomeFile As String) As Boolean
        Try
            ...
            If ConnettiAcc() Then
                ...
                'm_ConnAcc.Execute("delete * from ORV_RIGHE")
                m_ConnAcc.Execute("delete * from TOTALIZZATO")
            ...
            Dim dtArt As New DataTable("Anagrafica")
            accDataAd.Fill(dtArt)
            dsDest.Tables.Add(dtArt)


            accCommand.CommandText = "Select * from DBase"
            accDataAd.SelectCommand = accCommand
            Dim dtCdb As New DataTable("DBase")
            accDataAd.Fill(dtCdb)
            dsDest.Tables.Add(dtCdb)


            accCommand.CommandText = "Select * from ORV_TOT"
            accDataAd.SelectCommand = accCommand
            Dim dtOrvTOT As New DataTable("ORV_TOT")
            accDataAd.Fill(dtOrvTOT)
            dsDest.Tables.Add(dtOrvTOT)


            'accCommand.CommandText = "Select * from ORV_RIGHE"
            'accDataAd.SelectCommand = accCommand
            'Dim dtOrvR As New DataTable("ORV_RIGHE")
            'accDataAd.Fill(dtOrvR) 
            'dsDest.Tables.Add(dtOrvR) 


            accCommand.CommandText = "Select * from TOTALIZZATO"
            accDataAd.SelectCommand = accCommand
            Dim dtOraTOT As New DataTable("TOTALIZZATO")
            accDataAd.Fill(dtOraTOT)
            dsDest.Tables.Add(dtOraTOT)
            ...
            ''  Dim dtOrvT As New DataTable("ORV_TES")
            'Dim dtOrvR As New DataTable("ORV_RIGHE")

            'Dim dtOraTOT As New DataTable("ORA_TOT")

            '  Dim dtOraT As New DataTable("ORA_TES")
            '  Dim dtOraR As New DataTable("ORA_RIGHE")



            'dtArt.Columns.Add("ID")
            'dtArt.Columns.Add("Codart")
            'dtArt.Columns.Add("Descrizione")
            'dtArt.Columns.Add("GruppoCodifica")
            'dtArt.Columns.Add("TipoArt")
            'dtArt.Columns.Add("Marca")
            'dtArt.Columns.Add("CodForn")
            'dtArt.Columns.Add("CodArtForn")
            'dtArt.Columns.Add("NomeDb")

            'dtCdb.Columns.Add("ID")
            'dtCdb.Columns.Add("IDR")
            'dtCdb.Columns.Add("CodDb")
            'dtCdb.Columns.Add("CodArtCompo")
            'dtCdb.Columns.Add("Qta")
            'dtCdb.Columns.Add("Note")



            'dtOrvR.Columns.Add("ID_T")
            'dtOrvR.Columns.Add("IDR")
            'dtOrvR.Columns.Add("TIPO")
            'dtOrvR.Columns.Add("Codart")
            'dtOrvR.Columns.Add("CodArtCompo")
            'dtOrvR.Columns.Add("Qta")
            'dtOrvR.Columns.Add("CodCommessa")


            'dtOraTOT.Columns.Add("ID")
            'dtOraTOT.Columns.Add("CodForn")
            'dtOraTOT.Columns.Add("CODART")
            'dtOraTOT.Columns.Add("QTA")
            'dtOraTOT.Columns.Add("Prezzo")
            ...
            Dim Gr_PF As String = DatiGen.GrCod_PF
            Dim Gr_MP As String = DatiGen.GrCod_Comp  '"MP"

            'Dim CodCommessa As String = ""
            'CodCommessa = Trim(Replace(System.IO.Path.GetFileNameWithoutExtension(NomeFile), "TOTALIZZATO", ""))
            'If CodCommessa <> "" Then
            '    SalvaCommessa(DatiGen.DbGruppo, DatiGen.codCDA, CodCommessa)
            'End If

            Dim NrSchedeTot As Long = 0

            Dim SalvaCommessaEsolver As Boolean = False
            SalvaCommessaEsolver = GetSalvaComm()

            For i As Integer = 0 To ds.Tables.Count - 1
                Dim nometb As String = ds.Tables(i).TableName

                AggiornaLbl("Caricamento Dati in " & nometb)
                If UCase(nometb) = "DATI GENERALI" Then
                    idORV += 1
                    For r As Integer = 0 To ds.Tables(i).Rows.Count - 1
                        Try
                            Dim CodCli As String = ""
                            CodCli = Trim(NoNull(ds.Tables(i).Rows(r).Item(1)))
                            Dim CodArt As String = ""
                            CodArt = Trim(NoNull(ds.Tables(i).Rows(r).Item(2)))
                            Dim CodCommessa As String = ""
                            CodCommessa = Trim(NoNull(ds.Tables(i).Rows(r).Item(0)))
                            If CodCli <> "" Or CodArt <> "" Or CodCommessa <> "" Then
                                Dim mrowORV As DataRow = dtOrvTOT.NewRow
                                idRORV += 1
                                With mrowORV
                                    .Item("ID_T") = idORV
                                    .Item("IDR") = idRORV
                                    If SalvaCommessaEsolver Then
                                        If CodCommessa <> "" Then
                                            SalvaCommessa(DatiGen.DbGruppo, DatiGen.CodCDA, CodCommessa, CodCli)
                                        End If
                                    End If
                                    .Item("CodCommessa") = CodCommessa
                                    .Item("CodCli") = CodCli ' Trim(NoNull(ds.Tables(i).Rows(r).Item(1)))
                                    .Item("Codart") = CodArt 'Trim(NoNull(ds.Tables(i).Rows(r).Item(2)))
                                    .Item("Descrizione") = Trim(NoNull(ds.Tables(i).Rows(r).Item(3)))

                                    Dim ValoCol As String = Trim((NoNull(ds.Tables(i).Rows(r).Item(4))))
                                    Dim prz As Double = CDbl(IIf(ValoCol = "", 0, ValoCol))
                                    .Item("PrezzoUnit") = prz

                                    Dim datacon As String = Replace(Trim(NoNull(ds.Tables(i).Rows(r).Item(5))), ".", "/")
                                    'If datacon <> "" Then
                                    '    ' LogWrite("DATA confeziona : " & dataconf)
                                    '    'Dim datecon As Date = CDate(datacon)
                                    '    If IsDate(datacon) Then
                                    '        Dim mdata As Date = CDate(datacon)
                                    '        ' Dim mese As String = mdata.Month
                                    '        datacon = Format(mdata.Day, "00") & "/" & Format(mdata.Month, "00") & "/" & Format(mdata.Year, "0000") ' Format(CDate(dataconf), "dd/MM/yyyy")
                                    '    End If
                                    'End If

                                    If datacon <> "" Then
                                        LogWrite("DATA cons : " & datacon)
                                        'Dim datecon As Date = CDate(datacon)
                                        If IsDate(datacon) Then
                                            Dim mdata As Date = CDate(datacon)
                                            ' Dim mese As String = mdata.Month
                                            datacon = Mid(datacon, 1, 6) & Format(mdata.Year, "0000") ' Format(mdata.Day, "00") & "/" & Format(mdata.Month, "00") & "/" & Format(mdata.Year, "0000") ' Format(CDate(dataconf), "dd/MM/yyyy")
                                        Else
                                            datacon = Mid(datacon, 1, 6) & "20" & Mid(datacon, 7, 2)
                                        End If

                                        LogWrite("DATA cons : " & datacon)
                                    End If

                                    'LogWrite("DATA CONSEGNA : " & datacon)
                                    ''Dim datecon As Date = CDate(datacon)
                                    'If datacon <> "" Then
                                    '    If IsDate(datacon) Then 
                                    '        datacon = Format(CDate(datacon), "dd/MM/yyyy")
                                    '    End If
                                    'End If
                                    ' LogWrite("DATA CONSEGNA FORMAT: " & datacon)
                                    ' datacon = Format(CDate(datacon), "dd/MM/yyyy")
                                    .Item("DataConsegnaRich") = datacon
                                    .Item("TipoCommessa") = Trim(NoNull(ds.Tables(i).Rows(r).Item(6)))

                                    ValoCol = ""
                                    ValoCol = Trim((NoNull(ds.Tables(i).Rows(r).Item(7))))
                                    Dim qta As Double = CDbl(IIf(ValoCol = "", 0, ValoCol))
                                    .Item("Qta") = qta

                                    Dim datavers As String = Replace(Trim((NoNull(ds.Tables(i).Rows(r).Item(8)))), ".", "/")
                                    'If datavers <> "" Then
                                    '    ' LogWrite("DATA confeziona : " & dataconf)
                                    '    'Dim datecon As Date = CDate(datacon)
                                    '    If IsDate(datavers) Then
                                    '        Dim mdata As Date = CDate(datavers)
                                    '        ' Dim mese As String = mdata.Month
                                    '        datavers = Format(mdata.Day, "00") & "/" & Format(mdata.Month, "00") & "/" & Format(mdata.Year, "0000") ' Format(CDate(dataconf), "dd/MM/yyyy")
                                    '    End If
                                    'End If

                                    If datavers <> "" Then
                                        LogWrite("DATA vers : " & datavers)
                                        'Dim datecon As Date = CDate(datacon)
                                        If IsDate(datavers) Then
                                            Dim mdata As Date = CDate(datavers)
                                            ' Dim mese As String = mdata.Month
                                            datavers = Mid(datavers, 1, 6) & Format(mdata.Year, "0000") ' Format(mdata.Day, "00") & "/" & Format(mdata.Month, "00") & "/" & Format(mdata.Year, "0000") ' Format(CDate(dataconf), "dd/MM/yyyy")
                                        Else
                                            datavers = Mid(datavers, 1, 6) & "20" & Mid(datavers, 7, 2)
                                        End If

                                        LogWrite("DATA vers : " & datavers)
                                    End If

                                    ' If datavers <> "" Then datavers = Format(CDate(datavers), "dd/MM/yyyy")
                                    .Item("DbaseDataVersione") = datavers


                                    .Item("DBaseDescrizione") = Trim((NoNull(ds.Tables(i).Rows(r).Item(9))))

                                    'aggiunta del 30/01/2024
                                    Dim RifOrdCli As String = ""
                                    If ds.Tables(i).Columns.Count > 10 Then
                                        RifOrdCli = Trim((NoNull(ds.Tables(i).Rows(r).Item(10))))
                                    End If
                                    .Item("RiferimentoCli") = RifOrdCli
                                ...
                                End With
                                dtOrvTOT.Rows.Add(mrowORV)
                            End If
                        Catch ex As Exception
                            MsgBox("errore Scrittura " & nometb & " riga " & r & " :" & ex.Message & vbCrLf & "in " & ex.StackTrace)
                        End Try

                        'Dim mrowORVR As DataRow = dtOrvR.NewRow
                        'With mrowORVR
                        '    .Item("ID_T") = 1
                        '    .Item("IDR") = idRORV
                        '    .Item("TIPO") = "RIG"
                        '    .Item("Codart") = nometb
                        '    '   .Item("CodArtCompo") = "" ' Trim(ds.Tables(i).Rows(r).Item(0))
                        '    Dim ValoCol As String = Trim((NoNull(ds.Tables(i).Rows(r).Item(10))))
                        '    Dim qta As Double = CDbl(IIf(ValoCol = "", 0, ValoCol))
                        '    .Item("Qta") = qta
                        '    .Item("CodCommessa") = CodCommessa
                        'End With
                        'dtOrvR.Rows.Add(mrowORVR)
                    Next
                ElseIf UCase(nometb) <> "RIEPILOGO" And UCase(nometb) <> "TOTALIZZATO" Then
                    NrSchedeTot += 1
                    Dim SchedaPrincipale As String = ""
                    Dim mrowArt As DataRow
                    IdArt += 1
                    IdCdb += 1
                    IdRCdb = 0
                    For r As Integer = 0 To ds.Tables(i).Rows.Count - 1
                        Try
                            Dim Codart As String = Trim(NoNull(ds.Tables(i).Rows(r).Item(1)))
                            If Codart <> "" Then
                                'inserisco il prodotto finito e recupero il nome della scheda dalla prima riga 
                                'il nome del foglio è diverso dal codice della scheda (carattere / -> -) 
                                If r = 0 Then
                                    SchedaPrincipale = Trim(NoNull(ds.Tables(i).Rows(r).Item(2)))
                                    mrowArt = dtArt.NewRow
                                    With mrowArt
                                        .Item("Id") = IdArt
                                        .Item("CodArt") = SchedaPrincipale
                                        .Item("Descrizione") = SchedaPrincipale
                                        .Item("GruppoCodifica") = Gr_PF
                                        .Item("TipoArt") = 3 'prodotto finito "PF"
                                        .Item("UM") = ""
                                        .Item("Magazzino") = ""
                                        .Item("FAMIGLIA") = ""
                                        .Item("RifCircuito") = ""
                                        .Item("CodProduttore") = ""
                                        .Item("NomeDb") = SchedaPrincipale
                                        .Item("Scheda") = ""
                                        .Item("QtaTotLotto") = 1
                                        .Item("DbGruppo") = DatiGen.DbGruppo
                                    End With
                                    dtArt.Rows.Add(mrowArt)
                                End If
                                ...
                                With mrowArt
                                    IdArt += 1
                                    .Item("Id") = IdArt
                                    .Item("CodArt") = Trim(NoNull(ds.Tables(i).Rows(r).Item(1)))
                                    Dim Descr As String = Trim(NoNull(ds.Tables(i).Rows(r).Item(3)))
                                    'Dim Comment As String = Trim(NoNull(ds.Tables(i).Rows(r).Item(3)))
                                    .Item("Descrizione") = Descr ' IIf(Descr = "", Comment, Descr)
                                    .Item("GruppoCodifica") = Gr_MP
                                    .Item("TipoArt") = 0 ' materia prima "MP"
                                    .Item("UM") = Trim(NoNull(ds.Tables(i).Rows(r).Item(6)))
                                    .Item("Magazzino") = Trim(NoNull(ds.Tables(i).Rows(r).Item(7)))
                                    .Item("FAMIGLIA") = Trim(NoNull(ds.Tables(i).Rows(r).Item(8)))
                                    .Item("RifCircuito") = Trim(NoNull(ds.Tables(i).Rows(r).Item(9)))
                                    .Item("CodProduttore") = Trim(NoNull(ds.Tables(i).Rows(r).Item(10)))
                                    'FAMIGLIA -> famiglia ma poi esporto solo i primi 3 caratteri nella vista 
                                    '.Item("Marca") = Trim(NoNull(ds.Tables(i).Rows(r).Item(6)))
                                    '.Item("CodForn") = Date totalizzato AR
                                    '.Item("CodArtForn") = Date totalizzato As
                                    .Item("NomeDb") = Trim(NoNull(ds.Tables(i).Rows(r).Item(2))) ' nometb
                                    .Item("Scheda") = SchedaPrincipale
                                    Dim ValoCol As String = ""
                                    ValoCol = Trim(NoNull(ds.Tables(i).Rows(r).Item(5)))
                                    Dim qtatot As Double = CDbl(IIf(ValoCol = "", 0, ValoCol))
                                    .Item("QtaTotLotto") = qtatot
                                    'codice um tecnica acq e coeff acq da totalizzato colonna AT e AU
                                    .Item("DbGruppo") = DatiGen.DbGruppo
                                End With
                                dtArt.Rows.Add(mrowArt)

                                'inserisco Distinta base
                                Dim CodDb As String = Trim(NoNull(ds.Tables(i).Rows(r).Item(2)))
                                If CodDb <> "" Then
                                    Dim mrowCdb As DataRow = dtCdb.NewRow
                                    IdRCdb += 1
                                    With mrowCdb
                                        .Item("ID") = IdCdb
                                        .Item("IDR") = IdRCdb
                                        .Item("Livello") = Trim(NoNull(ds.Tables(i).Rows(r).Item(0)))
                                        .Item("CodArtCompo") = Trim(NoNull(ds.Tables(i).Rows(r).Item(1)))
                                        .Item("CodDb") = Trim(NoNull(ds.Tables(i).Rows(r).Item(2))) ' nometb
                                        Dim ValoCol As String = Trim(NoNull(ds.Tables(i).Rows(r).Item(4)))
                                        Dim qta As Double = CDbl(IIf(ValoCol = "", 0, ValoCol))
                                        .Item("Qta") = qta
                                        .Item("Scheda") = SchedaPrincipale
                                        ...
                                        Dim qtatot As Double = CDbl(IIf(ValoCol = "", 0, ValoCol))
                                        .Item("QtaTotLotto") = qtatot
                                        .Item("NoteDb") = Trim(NoNull(ds.Tables(i).Rows(r).Item(9)))
                                        .Item("DbGruppo") = DatiGen.DbGruppo
                                        '.Item("NoteDb") = colonna AS su totalizzato
                                        '.Item("ApprovvACarico")=0 se non trovo compilato la colonna AS su totalizzato ->2
                                        '  approvvigionamento a carico = 0 ->azienda
                                        '1->terzista
                                        '2->cliente
                                        'se non trovo compilato la colonna AS su totalizzato ->2
                                        'se tipo commessa C/P ->0
                                        'se tipo commessa C/L-> 2
                                        'se tipo commessa C/MISTO ->

                                        'esportare prima tutti i SMTT
                                        'Poi SMTB
                                        'Poi THT
                                    End With
                                    dtCdb.Rows.Add(mrowCdb)
                                End If

                                'ORV
                                'If r = 0 Then  'se prima riga scrivo la riga padre dell'ordine con qta della prima riga del foglio
                                '    Dim mrowORVR As DataRow = dtOrvR.NewRow
                                '    With mrowORVR
                                '        .Item("ID_T") = 1
                                '        .Item("IDR") = idRORV
                                ...
                                '        .Item("TIPO") = "CDB"
                                '        '   .Item("Codart") = nometb
                                '        .Item("Codart") = Trim(NoNull(ds.Tables(i).Rows(r).Item(0)))
                                '        Dim ValoCol As String = Trim((NoNull(ds.Tables(i).Rows(r).Item(10))))
                                '        Dim qta As Double = CDbl(IIf(ValoCol = "", 0, ValoCol))
                                '        .Item("Qta") = qta
                                '        .Item("CodCommessa") = Trim(Replace(System.IO.Path.GetFileNameWithoutExtension(NomeFile), "TOTALIZZATO", ""))
                    ...
                    Next
                Else
                    If UCase(nometb) = "TOTALIZZATO" Then
                        'se tipo commessa è conto lavoro C/L non creare l'ordine a fornitore

                        'dati da AP in poi per l'ordine di acquisto
                        For t As Integer = 1 To ds.Tables(i).Rows.Count - 1
                            Try
                                Dim mrowORatot As DataRow = dtOraTOT.NewRow
                                With mrowORatot
                                    .Item("ID") = t + 1
                                    .Item("CODART") = Trim(NoNull(ds.Tables(i).Rows(t).Item(0)))
                                    .Item("Descrizione") = Trim(NoNull(ds.Tables(i).Rows(t).Item(1)))
                                    .Item("FootPrint") = Trim(NoNull(ds.Tables(i).Rows(t).Item(2)))
                                    .Item("Manufacturer") = Trim(NoNull(ds.Tables(i).Rows(t).Item(3)))
                                    .Item("Manufacturer_PN") = Trim(NoNull(ds.Tables(i).Rows(t).Item(4)))
                                    .Item("Supplier") = Trim(NoNull(ds.Tables(i).Rows(t).Item(5)))
                                    .Item("SupplierPN") = Trim(NoNull(ds.Tables(i).Rows(t).Item(6)))
                                    .Item("SMT_THT") = Trim(NoNull(ds.Tables(i).Rows(t).Item(7)))
                                    '.Item("CodForn") = Trim(NoNull(ds.Tables(i).Rows(t).Item(6)))

                                    Dim Colonnatot As Integer = 8 + (ds.Tables.Count - 3)   'tutte le tabelle - dati generali e totalizzato per prendere il numero delle colonne da saltare
                                    'Dim mschede As String = ""
                                    'Dim NrSchede As Integer = ds.Tables.Count - 2
                                    'For schede As Integer = 0 To NrSchede
                                    '    Dim nomecol As String = ""
                                    '    Dim numerocol As Integer = 7 + schede + 1
                                    '    nomecol = ds.Tables(i).Columns.Item(numerocol).ColumnName
                                    '    Dim mrow As DataRow = ds.Tables(i).Rows(1)
                                    '    nomecol = ds.Tables(i).Rows(0).Item(numerocol)

                                    '    Dim ValCol As String = Trim(NoNull(ds.Tables(i).Rows(t).Item(numerocol)))
                                    '    Dim qtacol As Double = CDbl(IIf(ValCol = "", 0, ValCol))
                                    '    mschede = mschede & nomecol
                                    'Next
                                    Dim ValoCol As String = Trim(NoNull(ds.Tables(i).Rows(t).Item(Colonnatot)))
                                    Dim qta As Double = CDbl(IIf(ValoCol = "", 0, ValoCol))
                                    .Item("QTA") = qta
                                    .Item("Note") = Trim(NoNull(ds.Tables(i).Rows(t).Item(Colonnatot + 1)))


                                    Dim colonnaDatiForn As Integer = 40 + (ds.Tables.Count - 3)

                                    '.Item("Prezzo") = 0
                                    '.Item("Marca") = 0
                                    ValoCol = ""
                                    ValoCol = Trim(NoNull(ds.Tables(i).Rows(t).Item(colonnaDatiForn)))
                                    Dim prz As Double = CDbl(IIf(ValoCol = "", 0, ValoCol))
                                    .Item("PrezzoConf") = prz

                                    ValoCol = Trim(NoNull(ds.Tables(i).Rows(t).Item(colonnaDatiForn + 1)))
                                    .Item("TotaleUni") = CDbl(IIf(ValoCol = "", 0, ValoCol))
                                    .Item("CodForn") = Trim(NoNull(ds.Tables(i).Rows(t).Item(colonnaDatiForn + 2)))
                                    Dim ragsocforn As String = Trim(NoNull(ds.Tables(i).Rows(t).Item(colonnaDatiForn + 3)))
                                    .Item("CodArtForn") = Trim(NoNull(ds.Tables(i).Rows(t).Item(colonnaDatiForn + 4)))
                                    .Item("UmAcq") = Trim(NoNull(ds.Tables(i).Rows(t).Item(colonnaDatiForn + 5)))
                                    .Item("CoeffUm") = Trim(NoNull(ds.Tables(i).Rows(t).Item(colonnaDatiForn + 6)))
                                    Dim dataconf As String = Trim(NoNull(ds.Tables(i).Rows(t).Item(colonnaDatiForn + 7)))


                                    dataconf = Replace(Trim(dataconf), ".", "/")

                                    If dataconf <> "" Then
                                        LogWrite("DATA conf : " & dataconf)
                                        'Dim datecon As Date = CDate(datacon)
                                        If IsDate(dataconf) Then
                                            Dim mdata As Date = CDate(dataconf)
                                            ' Dim mese As String = mdata.Month
                                            dataconf = Mid(dataconf, 1, 6) & Format(mdata.Year, "0000") ' Format(mdata.Day, "00") & "/" & Format(mdata.Month, "00") & "/" & Format(mdata.Year, "0000") ' Format(CDate(dataconf), "dd/MM/yyyy")
                                        Else
                                            dataconf = Mid(dataconf, 1, 6) & "20" & Mid(dataconf, 7, 2)
                                        End If

                                        LogWrite("DATA conf : " & dataconf)
                                    End If

                                    ' If IsDate(dataconf) Then dataconf = Format(CDate(dataconf), "dd/MM/yyyy")
                                    .Item("DataConfermata") = dataconf
                                    .Item("MinOrd") = Trim(NoNull(ds.Tables(i).Rows(t).Item(colonnaDatiForn + 8)))

                                    ValoCol = Trim(NoNull(ds.Tables(i).Rows(t).Item(colonnaDatiForn + 9)))
                                    .Item("TotaleMinOrd") = CDbl(IIf(ValoCol = "", 0, ValoCol))
                                    ValoCol = Trim(NoNull(ds.Tables(i).Rows(t).Item(colonnaDatiForn + 10)))
                                    .Item("TotalePrezzo") = CDbl(IIf(ValoCol = "", 0, ValoCol))
                                    .Item("NoteForn") = Trim(NoNull(ds.Tables(i).Rows(t).Item(colonnaDatiForn + 11)))
                                    .Item("DbGruppo") = DatiGen.DbGruppo
                                    ''.Item("Prezzo") = 0 ' Trim(ds.Tables(i).Rows(t).Item(0))

                                    'Dim Descr As String = Trim(NoNull(ds.Tables(i).Rows(t).Item(2)))
                                    'Dim Comment As String = Trim(NoNull(ds.Tables(i).Rows(t).Item(1)))
                                    '.Item("Descrizione") = IIf(Descr = "", Comment, Descr)
                                    '.Item("Marca") = Trim(NoNull(ds.Tables(i).Rows(t).Item(4)))
                                    '.Item("CodForn") = Trim(NoNull(ds.Tables(i).Rows(t).Item(6)))
                                    '.Item("CodArtForn") = Trim(NoNull(ds.Tables(i).Rows(t).Item(7)))
                                    ''um colonna at

                                End With
                                dtOraTOT.Rows.Add(mrowORatot)


                            Catch ex As Exception
                                MsgBox("errore Scrittura " & nometb & " riga " & t & " :" & ex.Message & vbCrLf & "in " & ex.StackTrace)
                            End Try
                            ...
                            '    .Item("CodForn") = Trim(NoNull(ds.Tables(i).Rows(t).Item(6)))
                            '    .Item("CODART") = Trim(NoNull(ds.Tables(i).Rows(t).Item(0)))
                            '    Dim colonnaDatiForn As Integer = 40 + (ds.Tables.Count - 2)

                            '    Dim Colonnatot As Integer = 7 + (ds.Tables.Count - 2)   'tutte le tabelle - riepilogo e totalizzato per prendere il numero delle colonne da saltare
    ...
    Private Sub cmdRicPrev_Click(sender As System.Object, e As System.EventArgs) Handles cmdRicPrev.Click
        ...
        If IdDocPRV_RIF <> 0 Then
            txtRifPrev.Text = RiferimentoPrev
            txtRifPrev.Tag = IdDocPRV_RIF
        End If
    End Sub

    Private Sub cboTPLav_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles cboTPLav.SelectedIndexChanged
        Dim Tipolavoro As String = ""
        txtRifPrev.Enabled = False
        cmdRicPrev.Enabled = False
        Tipolavoro = cboTPLav.Text
        Select Case Mid(Tipolavoro, 1, 1)
            Case 1
                TP_LAVORO = TP_LAVORO_EN.TP_ANAGR
            Case 2
                TP_LAVORO = TP_LAVORO_EN.TP_OFFERTE
            Case 3
                TP_LAVORO = TP_LAVORO_EN.TP_ACCETTORV
        End Select
        ...
        If TP_LAVORO = TP_LAVORO_EN.TP_ACCETTORV Then
            ...
            txtRifPrev.Enabled = True
            cmdRicPrev.Enabled = True
        End If
        AbilitaEsportazione()
    End Sub

    Private Sub txtRifPrev_LostFocus(sender As Object, e As System.EventArgs) Handles txtRifPrev.LostFocus
        If Trim(txtRifPrev.Text) <> "" Then
            If Val(txtRifPrev.Tag) = 0 Then
                'recupero l'iddocumento
    ...
    Private Sub txtRifPrev_TextChanged(sender As System.Object, e As System.EventArgs) Handles txtRifPrev.TextChanged
